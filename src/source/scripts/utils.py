...
def writeText(surf, font, text, color, coor, alpha = 255, bold = False):
    font.set_bold(bold)
...
def inputHandler(shutdown = True, buttons = None, blocks = None): # manages the user's inputs
    for event in pygame.event.get():
        ...
        if cfg.releasenotes_btn.updating: cfg.releasenotes_btn.eventHandler(event)
        
        if cfg.releasenotes.render:
            cfg.releasenotes.exit_button.eventHandler(event)

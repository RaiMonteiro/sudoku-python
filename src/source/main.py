# ---------------- Sudoku Game by ----------------- #
# ----- Francisco Raí Monteiro, aka Cisco2000 ----- #
...
def main():
    ...
    cfg.FONT_CFG = [
        ...
        pygame.font.Font("./assets/font/YatraOne-Regular.ttf", 18)
    ]
...
def menu():
    ...
    while True:
        ...
        cfg.releasenotes_btn.draw() # release notes button
        if cfg.releasenotes.render: cfg.releasenotes.display() # release notes display
        cfg.c.showCursor()
        pygame.display.flip()

        if not cfg.releasenotes.render:
...
def shutdownPopUp():
    ...
    while True:
        u.setBg(1)
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], "Pretende sair do jogo?", cfg.BISQUE, (380, 220))
...
def gamePaused():
    ...
    while True:
        u.setBg(1)
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[3], "Em Pausa", cfg.BISQUE, (411, 200))
        u.drawObjects(cfg.btns)
        cfg.c.showCursor()
        pygame.display.flip()
...
def gameRequirements():
    ...
    while True: # defines the difficulty level
        u.setBg(1)
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], "Nível de Dificuldade", cfg.BISQUE, (411.5, 270))
        u.drawObjects(cfg.btns)
        cfg.c.showCursor()
        pygame.display.flip()
    ...
    while True: # defines user's name 
        u.setBg(1)
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], "Nome", cfg.BISQUE, (544.5, 250))
        ...
        if len(cfg.user_name) > 0:
            ...
            u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], cfg.user_name, cfg.BISQUE, (name_centralized.xAxis(), name_centralized.yAxis()))
...
def game():
    playing = True
    cfg.btns = [Button(cfg.DISPLAYSURF, "Pausar", (810, 280, 300, 70), 10, "game_pause")#,
                # Button(cfg.DISPLAYSURF, "?", (970, 280, 180, 70), 10, "game_help")
                ]
    ...
    def graphics():
        ...
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[0], f'Dificuldade: {cfg.DIFFICULTY[cfg.level][0]}', cfg.DARKBROWN, (800, 55)) # aside left
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[0], f'Erros: {cfg.errors}/3', cfg.DARKBROWN, (1020, 55)) # aside right
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], "Cronómetro", cfg.DARKBROWN, (u.Centralize((770, 70), (380, 160), cfg.FONT_CFG[2].size("Cronómetro")).xAxis(), 110)) # timer
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], cfg.timer.toStr(), cfg.DARKBROWN, (u.Centralize((770, 70), (380, 160), cfg.FONT_CFG[2].size(cfg.timer.toStr())).xAxis(), 170)) # time counter
...
def endGame():
    ...
    cfg.statistics.add([cfg.user_name, cfg.DIFFICULTY[cfg.level][0], cfg.timer.toStr(), cfg.errors, cfg.current_pts])
    ...
    while True:
        ...
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[2], cfg.user_name, cfg.BISQUE, (600 - cfg.FONT_CFG[2].size(cfg.user_name)[0] / 2, 150))
        u.writeText(cfg.DISPLAYSURF, cfg.FONT_CFG[3], str(counter), cfg.BISQUE, (600 - cfg.FONT_CFG[3].size(str(counter))[0] / 2, 200))

        u.writeText(new_surf, cfg.FONT_CFG[1], "Tempo", cfg.BISQUE, (551, 350), alpha)
        u.writeText(new_surf, cfg.FONT_CFG[2], cfg.timer.toStr(), cfg.BISQUE, (600 - cfg.FONT_CFG[2].size(cfg.timer.toStr())[0] / 2, 400), alpha)

        u.writeText(new_surf, cfg.FONT_CFG[1], "Dificuldade", cfg.BISQUE, (600-300-80.5, 350), alpha)
        u.writeText(new_surf, cfg.FONT_CFG[2], cfg.DIFFICULTY[cfg.level][0], cfg.BISQUE, (600-300 - cfg.FONT_CFG[2].size(cfg.DIFFICULTY[cfg.level][0])[0] / 2, 400), alpha)

        u.writeText(new_surf, cfg.FONT_CFG[1], "Erros", cfg.BISQUE, (600+300-41.5, 350), alpha)
        u.writeText(new_surf, cfg.FONT_CFG[2], str(cfg.errors), cfg.BISQUE, (600+300 - cfg.FONT_CFG[2].size(str(cfg.errors))[0] / 2, 400), alpha)
...
def scoreTable():
    x, y = (100, 70)
    box_w, box_h = (200, 50)
    players_data = cfg.statistics.unBoxData()
